...
alias gpull='git pull --autostash'
...
bw-unlock() {
    ...
    if [[ -n "$BW_SESSION" ]]; then
        echo "Bitwarden unlocked for this terminal session."
    else
...
set-default-browser() {
  ...
  if [[ -z "$desktop_file" ]]; then
    printf 'Usage: set-default-browser <browser.desktop>\n'
    return 2
  fi
...
alias use-brave='set-default-browser brave-browser.desktop'
alias use-chrome='set-default-browser google-chrome.desktop'
alias use-zen='set-default-browser app.zen_browser.zen.desktop'
alias use-firefox='set-default-browser firefox.desktop'
alias use-helium='set-default-browser helium.desktop'
alias use-min='set-default-browser min.desktop'

# Claude Code profiles
alias claude-mimo='CLAUDE_CONFIG_DIR="$HOME/.claude-mimo" claude'
alias claude-max='CLAUDE_CONFIG_DIR="$HOME/.claude-max" claude'
...
[[ $- != *i* ]] && return
_show_welcome_msg=1
[[ "$TERM_PROGRAM" == "vscode" || "$TERM_PROGRAM" == "cursor" || -n "$VSCODE_PID" || -n "$CURSOR_RECORD_SESSION" || "$CURSOR_INVOKED_AS" == "cursor-agent" ]] && _show_welcome_msg=0
...
if (( _show_welcome_msg )); then
  # Display welcome message on shell startup
  print_welcome_msg

  # Alias clear to redisplay welcome message for better UX
  alias clear='command clear && print_welcome_msg'
fi
unset _show_welcome_msg
